...
public final class BrightnessTileService extends TileService {
    ...
    public void onClick() {
        ...
        if (!Settings.System.canWrite(this)) {
            ...
            Toast.makeText(this, R.string.toast_open_app_for_brightness, Toast.LENGTH_LONG).show();
            updateTileView();
            return;
        }
        ...
        if (!AutoBrightnessManager.hasLightSensor(this)) {
            ...
            Toast.makeText(this, R.string.toast_sensor_unavailable, Toast.LENGTH_SHORT).show();
            updateTileView();
            return;
        }
    ...
    private void updateTileView() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            if (!enabled) {
                tile.setSubtitle(getString(R.string.tile_tap_to_protect));
            } else if (mode == AutoBrightnessManager.Mode.USER_HOLD) {
                tile.setSubtitle(getString(R.string.tile_holding, percent));
            } else {
                tile.setSubtitle(getString(R.string.tile_protected, percent));
            }
        }
